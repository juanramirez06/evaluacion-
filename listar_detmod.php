...
include 'pagination.php'; //incluir el archivo de paginación	
		...
		$sql = "SELECT id_detalle FROM detalles WHERE eli_detalle = '0' AND id_propiedad = '$id_propiedad' AND id_detalle > 0 ";
 		$conreg = $mysqli->query($sql);
		$numrows = mysqli_num_rows($conreg);
		$total_pages = ceil($numrows/$per_page);
		$reload = 'modify_property.php';
 
	
	$solofecha = date("Y-m-d");
	$sql = "SELECT detalles.*, caracteristicas.nom_carac FROM detalles INNER JOIN caracteristicas ON detalles.id_carac = caracteristicas.id_carac WHERE detalles.id_propiedad = '$id_propiedad' AND detalles.eli_detalle = '0' AND detalles.id_detalle > 0  ";
